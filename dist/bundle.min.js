...
'use strict';Object.defineProperty(exports,'__esModule',{value:!0});var _service=require('./service'),_service2=_interopRequireDefault(_service);function _interopRequireDefault(a){return a&&a.__esModule?a:{default:a}}exports.default=_service2.default,window.fsMetrics=_service2.default;
...
'use strict';Object.defineProperty(exports,'__esModule',{value:!0});var _protobuf=require('protobufjs/dist/light/protobuf.min'),_protobuf2=_interopRequireDefault(_protobuf),_metrics=require('./metrics.json'),_metrics2=_interopRequireDefault(_metrics),_featureDetectionService=require('./feature-detection-service.js'),_featureDetectionService2=_interopRequireDefault(_featureDetectionService);function _interopRequireDefault(a){return a&&a.__esModule?a:{default:a}}function invertObjectKV(a){const b={};for(let c in a)b[a[c]]=c;return b}function isDefined(a){return'undefined'!=typeof a}let metricsMessage=_protobuf2.default.Root.fromJSON(_metrics2.default).lookupType('fsMetrics.fsMetricsMessage');class FsMetrics{constructor({$window:d=window,$timeout:e=window.setTimeout,XMLHttpRequest:f=window.XMLHttpRequest,globalTags:g=[],timingRanges:h={},codeRevision:i='local',stringReplace:j=[],blackList:a,whiteList:b,timelinePrefix:k='fsm_',enabled:l=!0,bufferTime:m=5e3,metricNames:n=[],metricTags:o=[],eventTypes:p=['timing','set','gauge','increment','histogram'],timeParse:q=!1,metricsServer:c,metricPrefix:r=''}={}){this.$window=d,this.$timeout=e,this.XMLHttpRequest=f,this.globalTags=g||[],this.timingRanges=h,this.codeRevision=i,this.stringReplace=j,this.blackList=a,this.whiteList=b,this.timelinePrefix=k,this.enabled=l,this.bufTime=m,this.cfgMetricNames=n,this.cfgMetricTags=o,this.cfgEventTypes=p,this.timeParse=q,this.metricsServer=c,this.metricPrefix=r,this.loadFired=!1,this.oneTimeMetrics={},this.licenceUrls=[],this.resetQueue(),this.buildServerSharedDictionary(),this.trackUnsupportedFeatures(),this.upgradeQueue(),this.trackExistingPerformanceEntries()}trackExistingPerformanceEntries(){_featureDetectionService2.default.performanceTimeline&&(this.$window.performance.getEntries().forEach((a)=>{('measure'===a.entryType||'resource'===a.entryType)&&this.performanceTiming(a)}),'true'!==this.timeParse&&this.$window.performance.clearMeasures(),_featureDetectionService2.default.clearResourceTimings&&this.$window.performance.clearResourceTimings())}trackUnsupportedFeatures(){_featureDetectionService2.default.navigationTiming||this.increment('timingUnsupported'),_featureDetectionService2.default.mark||this.increment('markUnsupported'),_featureDetectionService2.default.measure||this.increment('markUnsupported'),_featureDetectionService2.default.performanceTimeline||this.increment('resourceTimingUnsupported')}buildServerSharedDictionary(){this.metricNames={},this.metricTags={},this.eventTypes={},this.cfgMetricNames&&this.cfgMetricNames.forEach((a,b)=>this.metricNames[a]=b),this.cfgMetricTags&&this.cfgMetricTags.forEach((a,b)=>this.metricTags[a]=''+b),this.cfgEventTypes&&this.cfgEventTypes.forEach((a,b)=>this.eventTypes[a]=b)}upgradeQueue(){this.$window.fsMetrics&&Array.isArray(this.$window.fsMetrics)&&Array.prototype.push===this.$window.fsMetrics.push&&(this.$window.fsMetrics.push=this.convertWindowQueue.bind(this),this.$window.fsMetrics.forEach(this.$window.fsMetrics.push))}convertWindowQueue([a,b,c=1,d={},e=!1]){if('pageload'!==a)'string'==typeof a&&(a=this.eventTypes[a]),this.queueMetric(a,b,c,d,e);else if(d.dest=b,_featureDetectionService2.default.navigationTiming){var f=window.performance.timing.domContentLoadedEventStart-window.performance.timing.navigationStart;1.2e5>f?this.timing('domContentLoaded',f,d,!0):this.increment('timingOutlier',d,!0)}else this.increment('timingUnsupported',d,!0)}resetQueue(){this.queue=[],this.activeBuffer&&this.$window.clearTimeout(this.activeBuffer),this.activeBuffer=!1}normalizeTag(a){return a+='',this.stringReplace.reduce((a,[b,c])=>a.replace(b,c),a)}encodeStat(...a){return a.reduce((a,b)=>b?(b+='',a+b.replace(/[^\w:.-]+/g,'_')):b&&isDefined(this.metricNames[b])?this.metricNames[b]:a,'')}encodeTagsIntoMetricName(a,b){let c='';return b&&(c='|'+JSON.stringify(b)),a+c}measureSafe(a,b,c,d){if(_featureDetectionService2.default.measure&&_featureDetectionService2.default.timing){b in window.performance.timing||(b=this.timelinePrefix+b),c in window.performance.timing||(c=this.timelinePrefix+c);try{this.$window.performance.measure(this.timelinePrefix+this.encodeTagsIntoMetricName(a,d),b,c)}catch(a){console.error(a)}}}timingEvent(a,b){if(_featureDetectionService2.default.measure&&_featureDetectionService2.default.mark&&(this.$window.performance.mark(this.timelinePrefix+a),!!this.timingRanges[a]))for(let{from:c,metric:d,once:e}of this.timingRanges[a])if(!e||!this.oneTimeMetrics[d]){if(e&&(this.oneTimeMetrics[d]=!0),'stateChange.start'===a&&'domContentLoadedEventStart'===c)return void this.timing('timeToFirstRoute',this.$window.performance.now()-this.$window.performance.timing.domContentLoadedEventStart);this.measureSafe(d,c,a,b)}}increment(a,b={},c=!1){this.queueMetric(this.eventTypes.increment,a,1,b,c)}histogram(a,b,c={},d=!1){this.queueMetric(this.eventTypes.histogram,a,b,c,d)}timing(a,b,c={},d=!1){this.queueMetric(this.eventTypes.timing,a,b,c,d)}performanceTiming(a){if(!this.whiteList.test(a.name)||this.blackList.test(a.name))return void console.log(a.name);if('resource'===a.entryType)'xmlhttprequest'===a.initiatorType&&this.licenceUrls.includes(a.name)?this.trackLicenseReq(a):this.timing('resourceTiming',a.duration,{url:a.name,initiatorType:a.initiatorType,cached:0!==a.decodedBodySize&&0===a.transferSize});else if('measure'===a.entryType){let[b,c]=a.name.split('|',2);b=b.split(this.timelinePrefix,2).pop(),c&&(c=JSON.parse(c)),this.timing(b,a.duration,c)}}queueMetric(a='type is undefined',b='key is undefined',c,d={},e=!1){return!this.enabled||this.$window.navigator.doNotTrack?!1:0===c||c?a===this.eventTypes.timing&&(!this.$window.isFinite(c)||1.2e5<c||0>c)?void this.increment('timingOutlier'):void(this.queue.push({type:a,key:b,val:c,tags:d}),e?this.flushMetrics():!this.activeBuffer&&(this.activeBuffer=this.$window.setTimeout(this.flushMetrics.bind(this),this.bufTime||5e3))):void 0}encodeTags(a){const b=[];for(let c in a)b.push(`${c}:${this.normalizeTag(a[c])}`);return b}buildDict(a,b){return b[a]||(b[a]=b.id++),b[a]}encodeMetric(a,b){(0===a.type||3===a.type||5===a.type)&&(a.val|=0),a.tags=this.encodeTags(a.tags),a.key=this.encodeStat(a.key),a.key=this.buildDict(a.key,b),a.tags=a.tags.map((a)=>this.buildDict(a,b)),'string'==typeof a.val&&(a.val=this.buildDict(a.val,b))}preparePayload(a){const b={codeSha:this.codeRevision,globalTag:this.globalTags.slice(),aMetric:[],prefix:this.metricPrefix},c={};Object.defineProperty(c,'id',{value:0,writable:!0,configurable:!1,enumerable:!1});const d={0:'metricValueTiming',1:'metricValueSet',2:'metricValueGauge',3:'metricValueIncrement',4:'metricValueHistogram',5:'metricValueIncrement',6:'metricValueEvent'};for(let e of a)this.encodeMetric(e,c),b.aMetric.push({tag:e.tags,type:e.type,metricName:e.key,[d[e.type]]:e.val});b.stringDict=invertObjectKV(c),console.log(b.stringDict),console.log(b);const e=metricsMessage.verify(b);return e?void console.error(e):metricsMessage.encode(metricsMessage.create(b)).finish()}flushMetrics(){const a=new this.XMLHttpRequest;a.open('PUT',this.metricsServer),a.setRequestHeader('Content-Type','application/octet-stream');const b=this.preparePayload(this.queue);b&&a.send(b),this.resetQueue()}}exports.default=FsMetrics;

},{"./feature-detection-service.js":2,"./metrics.json":4,"protobufjs/dist/light/protobuf.min":1}]},{},[3]);
